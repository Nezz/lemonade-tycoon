import { AchievementId, AchievementDefinition, GameState, DayResult, UpgradeId } from './types';
...
export function checkAchievements(
  ...
  if (allResults.length >= 7) {
    ...
    check('pennyPincher', last7.every((r) => r.profit > 0));
  }

  // Event Survivor: profit on a negative-demand event day
  const negativeEvents = ['construction', 'competingStand', 'rainSurprise', 'lemonShortage', 'powerOutage'];
  check(
    'eventSurvivor',
    result.event !== null &&
